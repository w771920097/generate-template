  ...
  protected String building() {
    ...
    for (int i = 0; i < dataMetaList.size(); i++) {
      ...
      if (field.endsWith("Date")) {
        ...
        sb.append("\t\t\t<em class=\"form-req\">*</em> <label class=\"form-lb1\">" + commont
            + "：</label>\n");
        sb.append("\t\t</div>\n");
        sb.append("\t\t<div class=\"grid_7\">\n");
        sb.append("\t\t\t<input type=\"text\" name=\"" + field + "\" id=\"" + field
            + "\" maxlength=\"" + maxlength + "\"\n");
    ...
    sb.append("\t\t\t\t\t\t\t\"/" + basicData.getDomainName() + "/add"
        + basicData.getUpperCaseDomainName() + "\");\n");
    sb.append("\t\t\t\t} else if (mode == \"edit\") {\n");
    sb.append("\t\t\t\t\t$(\"#maintainForm\").attr(\"action\",\n");
    sb.append("\t\t\t\t\t\t\t\"/" + basicData.getDomainName() + "/update"
        + basicData.getUpperCaseDomainName() + "\");\n");
    ...
    sb.append("\t\t\t\t\t\t\t\t\t\t\t\t\t$.messageBox({\n");
    sb.append(
        "\t\t\t\t\t\t\t\t\t\t\t\t\t\tmessage : \"新增" + basicData.getDomainCname() + "成功!\"\n");
    sb.append("\t\t\t\t\t\t\t\t\t\t\t\t\t});\n");
    sb.append("\t\t\t\t\t\t\t\t\t\t\t\t} else if (mode == \"update\") {\n");
    sb.append("\t\t\t\t\t\t\t\t\t\t\t\t\t$.messageBox({\n");
    sb.append(
        "\t\t\t\t\t\t\t\t\t\t\t\t\t\tmessage : \"修改" + basicData.getDomainCname() + "成功!\"\n");
    sb.append("\t\t\t\t\t\t\t\t\t\t\t\t\t});\n");
    sb.append("\t\t\t\t\t\t\t\t\t\t\t\t}\n");
    sb.append("\n");
